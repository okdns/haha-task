 ...
 *   本软件基本功能和流程都是可用的，因为没经过更多用户的使用测试，可能难免会存在一些问题，所以版本号暂定为0.9
 **/

/*----config ---*/
	define('HAHA_VER', '0.9.1');		//版本号
	define('HAHA_TITLE','Task/bug管理系统');
	define('IS_DEV', 1);				//当前环境，开发环境能输出调试信息和详细报错
	error_reporting(IS_DEV?E_ALL:0);

	$dsn	= 'sqlite://./haha_task.db'; //sqlite数据库
	//$dsn	= 'mysql://root:123456@localhost:3306/haha_db?charset=utf8'; //mysql数据库

	$enable_up_file	= true;			//是否允许上传图片
	$save_url		= '/upload';	//图片文件夹的url
	$save_path		= __DIR__.'/upload'; //图片保存文件夹的路径
	$img_type_list		= ['jpg','png','jpeg','gif','webp'];//支持的图片类型
	$display_max_width	= 450;		//图片显示的最大宽度(上传图片长宽未做限制，这里是限制显示时的宽度)
...
/*------------*/
	...
	$task_role = [1 => '普通员工',10=>'程序开发',20=>'美工设计',50=>'推广',60=>'客服',70=>'产品经理', 80=>'行政经理', 99=>'超级管理员'];
...
/* ==== main =====*/
	...
	$haha_model = new haha_model($dsn);
	if(!$haha_model) die($haha_model->error_info());
	if(!$haha_model->exists_table()){
		if(isset($_POST['tab']) && $post['tab']='init'){
			$haha_model->init_table();
	...
	if(isset($_SESSION['login_user'])){
		...
		if(isset($_COOKIE['haha_login_token'])){
			...
			$user		= $haha_model->get_user($token_arr[0]);
	...
	$users		= $haha_model->list_user(1);	//全部成员id、name
	$work_users	= $users;						//可处理任务的人员id、name
	$manager_uids	= $haha_model->get_uids_by_role([70,80,99]); //有任务管理权限的uid列表
	...
	if($_POST){
		...
		if($tab=='login'){
			...
			$user		= $haha_model->get_user_by_account($account);
		...
		if($tab=='user_del'){
			...
			if($uid){
				if($uid==1){
					...
					$haha_model->delete('haha_user', ['uid'=>$uid]);
		...
		if($tab=='user_add'){
			...
			if($haha_model->get_user_by_account($account)){
				echo json_encode(['status'=>0,'msg'=>haha_lang('user_tip_namefind')]); exit;
			}
			if($haha_model->get_user_by_nickname($nickname)){
				echo json_encode(['status'=>0,'msg'=>haha_lang('user_tip_nickfind')]); exit;
			}
			$uid = $haha_model->add_user($account,$password,$nickname,$role,$status);
			if($uid){
		...
		if($tab=='user_edit'){
			...
			if(isset($_POST['account'])){
				$account =  v_post('account','');
				if($account){
					if($haha_model->get_user_by_account($account, $uid)){
						echo json_encode(['status'=>0,'msg'=>haha_lang('user_tip_namefind')]); exit;
					}
			...
			if(isset($_POST['nickname'])){
				$nickname = v_post('nickname','');
				if($nickname){
					$find = $haha_model->get_user_by_nickname($nickname, $uid);
					if($find){
						echo json_encode(['status'=>0,'msg'=>haha_lang('user_tip_nickfind')]); exit;
					}
			...
			$res = $haha_model->update('haha_user', ['uid'=>$uid], $data);
			if($res){
				echo json_encode(['status'=>1,'msg'=>haha_lang('tip_succeed')]); exit;
			}else{
				echo json_encode(['status'=>0,'msg'=>haha_lang('tip_failed')]); exit;
		...
		if($tab=='modify_pswd'){
			$password = v_post('password','');
			if($password){
				$haha_model->modify_password($login_user['uid'], $password);
		...
		if($tab=="clear_msg"){
			$haha_model->clear_msg($login_user['uid']);
		...
		if($tab=='add'){//【添加】 权限:所有人；通知：处理者，审核者
			...
			$inst_id = $haha_model->add_task($data);
			if($inst_id){
				$haha_model->add_log($inst_id, 'add', $login_user['uid']);
			}
			$msg_type = v_post('msg_type',0);
			$msg_url = '';
			if('notice'==$msg_type && $post_status==9){//是从欢迎页添加完成公告方式提交的
				$msg_content = str_limit(strip_tags($post_note),30);
				$haha_model->add_notice($msg_content,$login_user['uid'],'');
			}else{
				$sended_uids = array();
				if($post_acceptor_uid && $post_acceptor_uid!=$login_user['uid'] && !in_array($post_acceptor_uid, $sended_uids)) {
					$haha_model->add_msg($post_acceptor_uid,$login_user['nickname'].haha_lang('msg_added',$inst_id).', '.haha_lang('msg_add_test'), 0,$msg_url, $msg_target);
					$sended_uids[]=$post_acceptor_uid;
				}
				if($post_worker_uid && $post_worker_uid!=$login_user['uid'] && !in_array($post_worker_uid, $sended_uids)){
					$haha_model->add_msg($post_worker_uid,$login_user['nickname'].haha_lang('msg_added',$inst_id).haha_lang('msg_add_to'), 0, $msg_url, $msg_target);
					$sended_uids[]=$post_worker_uid;
				}
			}
			$back = array('status'=>1,'msg'=>haha_lang('tip_add'));
		}else{
			$task = $haha_model->get_task($task_id);
			$log_arr = $haha_model->list_log($task_id);
			$follow_uids = $haha_model->get_follow_uids($task_id);
			$msg_content = '';
			if($tab=='view'){//【查看】 权限:所有人
				...
				if(!in_array($login_user['uid'],$view_list)){
					...
					//$haha_model->update('haha_task',['id'=>$task_id],['view'=>$view_json]);
					$task['view'] = $view_json;
				}
				...
				if($task){
					$haha_model->get_work($task_id, $login_user['uid']);
				...
				if(in_array($login_user['uid'], array_merge($manager_uids,array($task['submiter_uid'], $task['acceptor_uid'])))) {
					$haha_model->end_work($task_id, $login_user['uid']);
				...
				if(in_array($login_user['uid'], array_merge($manager_uids,array($task['submiter_uid'], $task['acceptor_uid'])))) {
					$haha_model->no_work($task_id, $login_user['uid']);
					if($task['worker_uid'] && $task['worker_uid']!=$login_user['uid']){
						$haha_model->add_msg($task['worker_uid'],haha_lang('msg_my_no',$task_id,$login_user['nickname']),0, $msg_url, $msg_target);
				...
				if(in_array($login_user['uid'], array_merge($manager_uids,array($task['submiter_uid'], $task['worker_uid'])))) {
					$haha_model->ok_work($task_id, $login_user['uid']);
				...
				if(in_array($login_user['uid'], array_merge($manager_uids,array($task['submiter_uid'])))) {
					$haha_model->open_work($task_id, $login_user['uid']);
					if($task['worker_uid'] && $task['worker_uid']!=$login_user['uid']){
						$haha_model->add_msg($task['worker_uid'],haha_lang('msg_my_open',$login_user['nickname'],$task_id),0, $msg_url, $msg_target);
						$follow_uids = array_diff($follow_uids, array($task['worker_uid']));
					}
				...
				if(in_array($login_user['uid'], array_merge($manager_uids,array($task['submiter_uid'])))) {
					$haha_model->cancel_work($task_id, $login_user['uid']);
				...
				if(in_array($login_user['uid'], array_merge($manager_uids,array($task['submiter_uid'])))) {
					$haha_model->del_work($task_id, $login_user['uid']);
					$msg_url = '';
				...
				if($task_id && $field){
					if(in_array($login_user['uid'], array_merge($manager_uids,array($task['submiter_uid'], $task['acceptor_uid'])))) {
						if(1){
							if($field == 'worker_uid') {
								$haha_model->edit_task($task_id, array($field=>$value,'status'=>($task['status']==1?7:$task['status'])));
								$haha_model->add_log($task_id, 'to', $login_user['uid'], $value);
								if($value!=$login_user['uid']){
									//if($value && !in_array($value,$sended_uid)){ //
									//	$haha_model->add_msg($value, haha_lang('msg_to_me',$login_user['nickname'],$task_id),0, $msg_url, $msg_target);
									//	$sended_uid[]=$value;
									//}
								...
								$haha_model->edit_task($task_id, [$field=>$value]);
								$haha_model->add_log($task_id, 'assign', $login_user['uid'], $value);
								if($value!=$login_user['uid']){
									//if($value && !in_array($value,$sended_uid)){
									//	$haha_model->add_msg($value, haha_lang('msg_test_me',$login_user['nickname'],$task_id),0, $msg_url, $msg_target);
									//	$sended_uid[]=$value;
									//}
								...
								$haha_model->edit_task($task_id, $update);
								if($field_data){
									$msg_content = haha_lang('msg_edit2',$login_user['nickname'],$task_id,$field_title,$field_data);
								}else{
									$msg_content = haha_lang('msg_edit',$login_user['nickname'],$task_id,$field_title);
								}
								if($task['worker_uid'] && $task['worker_uid'] != $login_user['uid'] && !in_array($task['worker_uid'], $follow_uids)){
									$follow_uids[]=$task['worker_uid'];
								}
							...
							if($task['submiter_uid'] != $login_user['uid']){
								$haha_model->add_msg($task['submiter_uid'],haha_lang('tip_modify'),0, $msg_url, $msg_target);
							}
						}
						$back = array('status'=>1,'msg'=>haha_lang('tip_success'));
					}else{
				...
				if($task_id && $field=='remark'){
					if(in_array($login_user['uid'], array_merge($manager_uids,array($task['submiter_uid'], $task['worker_uid'], $task['acceptor_uid'])))) {
						...
						$haha_model->add_remark($task_id, $login_user['uid'], $value);
						if($task['worker_uid'] && $task['worker_uid']!=$login_user['uid'] && !in_array($task['worker_uid'], $follow_uids)){
							$follow_uids[]=$task['worker_uid'];
						}
						if($task['acceptor_uid'] && $task['acceptor_uid']!=$login_user['uid'] && !in_array($task['acceptor_uid'], $follow_uids)){
				...
				$follow_uids = $haha_model->follow_work($task_id, $login_user['uid'], $follow_status);
			...
			if($follow_uids && $msg_content){
				...
				foreach($follow_uids as $msg_to_uid){
					$haha_model->add_msg($msg_to_uid, $msg_content, 0, $msg_url, $msg_target);
				}
			}
		}
		if(empty($back)) {
	...
	if(in_array($tab,array('add','view','edit'))){
		...
		$list_task = $haha_model->list_task($where_arr, $order_arr, $select_field, $limit);
	}
	$my_tab = $tab;
...
?>
	<?php if(in_array($my_tab,array('add','edit','view'))):?>
		<?php
			$remark_list	= $haha_model->list_remark($task_id);
			$log_list		= $haha_model->list_log($task_id);
			$follow_list	= $haha_model->list_follow($task_id);
		?>

		<script src="?haha_res=nicEdit.js&ver=<?=HAHA_VER?>"></script>
		<script>
	...
	<?php elseif($my_tab=='msg'):?>
		...
		<div class="msg_list">
			...
			<?php 
				$msgs = $haha_model->list_msg($login_user['uid']);
			?>
			<ul>
			<?php if($msgs):?>
	...
	<?php elseif($my_tab=='users'):?>
		<?php
			$users = $haha_model->list_user(0,[],'uid desc');
		?>
		<div class="clearfix">
		...
		<table class="list_table" style="margin-top:10px;">
			...
			<?php foreach($users as $r):?>
				<tr>
					<td><?=$r['uid']?></td>
					<td <?php if($r['status']==0):?>style="color:#999; text-decoration:line-through;"<?php endif;?>><?=$r['nickname']?></td>
					<td <?php if($r['status']==0):?>style="color:#999; text-decoration:line-through;"<?php endif;?>><?=$r['account']?></td>
					...
					<td style="text-align:center;">
						<?php if(in_array($login_user['uid'], $manager_uids) || $login_user['role']==99):?>
						...
						<?php endif;?>
						<?php if($r['status']==1):?>
						<a href="javascript:;" onclick="popup('?tab=user_msg&uid=<?=$r['uid']?>')" ><i class="icon haha-task icon-notice"></i><?=haha_lang('btn_user_msg')?></a>
						<?php endif;?>
					</td>
				</tr>
			<?php endforeach;?>
	...
	<?php elseif($my_tab=='edit_user'):?>
		<?php
			$uid = v_get('uid',0);
			$user = $uid?$haha_model->get_user($uid):[];
		?>
	...
	<?php else:?>
		<?php
			$count_wait = $haha_model->count_task(['status'=>1]);
			$count_test = $haha_model->count_task(['status'=>4]);
			$count_msg	= $haha_model->count_msg($login_user['uid']);
		...
		<?php if($my_tab=='stats'):?>
			...
			<?php
				$num_wait = $haha_model->count_task(['status'=>1]);
				$num_work = $haha_model->count_task(['status'=>7]);
				$num_test = $haha_model->count_task(['status'=>4]);
				$num_curr = $num_wait + $num_work + $num_test;
				$num_end = $haha_model->count_task(['status'=>9]);
				$num_cancel = $haha_model->count_task(['status'=>11]);
			?>
			<ul class="stats">
				<dl>
			...
			<?php
				$rank_worker_now	= $haha_model->count_rank('worker_uid',['status'=>7]);
				$rank_worker_end	= $haha_model->count_rank('worker_uid',['status'=>9]);
				$rank_submiter		= $haha_model->count_rank('submiter_uid',['status'=>[1,4,7,9]]);
				$rank_acceptor		= $haha_model->count_rank('acceptor_uid',['status'=>[1,4,7,9]]);
			?>
			...
			<div style="clear:both;">
				...
				<?php endif;?>
				<?php if($rank_acceptor):?>
				<?php $rank_acceptor_data = make_user_rank_data($rank_acceptor, $users);?>
				<div class="canvas-wrapper"><canvas id="tu_rank_acceptor"></canvas></div>
				<script>
					new Schart('tu_rank_acceptor', {
						type: 'pie',
						title: {text: '<?=haha_lang('label_rank_acceptor')?>'},
						legend: {position: 'left'},
						bgColor: '#fbfbfb',
						labels: <?=json_encode($rank_acceptor_data['names'])?>,
						datasets: [{
							data: <?=json_encode($rank_acceptor_data['values'])?>
						}]
					});
				</script>
				<?php endif;?>
...
class sql_db {
	...
	function __construct($dsn) {
		$db_conf = parse_url($dsn);
		if(isset($db_conf['scheme'])){
			$this->db_name = substr($db_conf['path'],1);
			if('sqlite'==$db_conf['scheme']){ //sqlite 数据库
				$this->db_type='sqlite';
				$this->db = new SQLite3($this->db_name);
			}elseif('mysql'==$db_conf['scheme']){ //mysql 数据库
				$this->db_type='mysql';
				$port = isset($db_conf['port'])?$db_conf['port']:3306;
				$this->db = @new mysqli($db_conf['host'], $db_conf['user'], $db_conf['pass'], '', $port);
				if($this->db->connect_error) {
					die('Connect Error: '.$this->db->connect_error);
				}
				$this->db->select_db($this->db_name);
				if(isset($db_conf['query'])){
					parse_str($db_conf['query'], $query_arr);
					if(isset($query_arr['charset'])) $this->db->set_charset($query_arr['charset']);
				}
			}else{
				die(haha_lang('tip_dsn_err'));
			}
		}else{
			die(haha_lang('tip_dsn_err'));
		}
	...
	function select_db($db_name){
		return $this->db->select_db($db_name);
	}
	...
	function exists_table($table='haha_task'){
		if($this->db_type=='sqlite'){
			...
			$sql = "SELECT count(*) FROM information_schema.tables WHERE table_schema='".$this->db_name."' AND table_name='{$table}'";
		}
	...
	function get_indexs($table){
		if($this->db_type=='sqlite'){
			$arr = $this->get_col("SELECT `name` FROM sqlite_master WHERE type='index' AND tbl_name='{$table}'");
		}else{
			$arr = $this->get_col("SHOW INDEX FROM {$table}",'Column_name');
		}
		return $arr;
	}
...
class haha_model extends sql_db{
	...
	function init_table(){
		...
		'haha_user' => "
			CREATE TABLE `haha_user` (
				...
				`nickname` varchar(16) NOT NULL DEFAULT '',      -- 呢称
		...
		'haha_msg' => "
			CREATE TABLE `haha_msg` (
				...
				`type` int NOT NULL DEFAULT '0',                  -- 类别
				`url` varchar(64) NOT NULL DEFAULT '',           -- 链接
				`ctime` int NOT NULL DEFAULT '0',                 -- 生成时间
				`rtime` int NOT NULL DEFAULT '0'                  -- 阅览时间
			);",
		];
		if('mysql'==$this->db_type){
			if(false===$this->select_db($this->db_name)){
				$this->create_db($this->db_name);
				$this->select_db($this->db_name);
			}
		}
		foreach($tables as $tb){
			if('mysql'==$this->db_type){
				$tb = str_replace('AUTOINCREMENT','AUTO_INCREMENT',$tb);
			}
			$this->db->query($tb);
		}
		$list_index = [
	...
	function modify_password($uid, $password){
		return $this->update('haha_user', ['uid'=>$uid], ['password'=>md5($password)]);
	}
	...
	function count_rank($group_field, $where_arr=null){
		...
		$where = $this->make_where_sql($where_arr);
		$sql.= $where?" WHERE ".$where." AND ".$group_field." != 0" :'';
...
function res_encode($str){
	return str_replace("'", '#@',str_replace("\\", '#&',str_replace('$', '#%',$str)));
}

function res_decode($str){
	return str_replace('#%', '$', str_replace('#&', "\\", str_replace('#@', "'",$str)));
}
...
function haha_lang($key){
	$lang = array(
		...
		'msg_edit'		=> '{str}已经修改了工作任务[#{str2}]的{str3}',
		'msg_edit2'		=> '{str}已经修改了工作任务[#{str2}]的{str3}为{str4}',
		'msg_cleared'	=> '清理完成',
		'confirm'		=> '确认{str}吗？',
		'tip_empty'		=> '请输入内容！',
		'tip_success'	=> '提交成功',
		...
		'label_rank_submiter'	=> '发布任务数排名：',
		'label_rank_acceptor'	=> '测审工作量排名：',
		'view_type'		=> '选择列表界面样式',
		'time_axis'		=> '时间轴',
		'follower'		=> '关注者：',
...
function haha_html($html){
	$list = [
		...
		'login' => "
			...
			<link rel='icon' href='?haha_res=favicon.png&ver=".HAHA_VER."'>
			<style>
		...
		'common_head' => "<!DOCTYPE html><html><head><meta charset='utf-8'>
			...
			<script src='?haha_res=jquery.min.js&ver=".HAHA_VER."'></script>
			<link rel='icon' href='?haha_res=favicon.png&ver=".HAHA_VER."' />
			<link rel='stylesheet' href='?haha_res=style.css&ver=".HAHA_VER."' />
			<script>
...
function haha_res($filename){
	...
	'style.css'=>"
		*{outline: 0;margin: 0; padding: 0;}
		...
		.btn_bg7{background-image: url('?haha_res=btn_bg7.png&ver=".HAHA_VER."');}
		.edit_flat_bg0{background-image: url('?haha_res=task_flat_bg10.png&ver=".HAHA_VER."');}
		.task_flat_bg0{background-image: url('?haha_res=task_flat_bg0.png&ver=".HAHA_VER."');}
		.task_flat_bg1,.edit_flat_bg1{background-image: url('?haha_res=task_flat_bg1.png&ver=".HAHA_VER."');}
		.task_flat_bg2,.edit_flat_bg2{background-image: url('?haha_res=task_flat_bg2.png&ver=".HAHA_VER."');}
		.task_flat_bg3,.edit_flat_bg3{background-image: url('?haha_res=task_flat_bg3.png&ver=".HAHA_VER."');}
		.task_flat_bg4,.edit_flat_bg4{background-image: url('?haha_res=task_flat_bg4.png&ver=".HAHA_VER."');}
		...
		.task_sticky_bg0{background-image: url('?haha_res=task_sticky_bg0.png&ver=".HAHA_VER."');}
		.task_sticky_bg1{background-image: url('?haha_res=task_sticky_bg1.png&ver=".HAHA_VER."');}
		.task_sticky_bg2{background-image: url('?haha_res=task_sticky_bg2.png&ver=".HAHA_VER."');}
		.task_sticky_bg3{background-image: url('?haha_res=task_sticky_bg3.png&ver=".HAHA_VER."');}
		.task_sticky_bg4{background-image: url('?haha_res=task_sticky_bg4.png&ver=".HAHA_VER."');}

		@font-face {font-family: 'haha-task'; src: url('?haha_res=haha_fonts.woff&ver=".HAHA_VER."') format('woff');}
		...
		#cover{position:fixed; left:0; top:0; background:url('?haha_res=pop_bg.jpg&ver=".HAHA_VER."'); width:100%; height:100%; z-index:100; display:none;}
		...
		#popup a{position:absolute; width:57px; height:57px; right:0; top:0; margin-top:-20px;margin-right:-20px; background:url('?haha_res=pop_btn.png&ver=".HAHA_VER."');cursor:pointer;}
	...
	$types=[
		'js'	=> ['mime'=>'text/JavaScript','res_decode'=>1],
		'css'	=> ['mime'=>'text/css'],
	...
	if($type && isset($list[$filename]) && isset($types[$type])) {
		...
		if(isset($types[$type]['res_decode'])){
			echo res_decode($list[$filename]);
		}elseif(isset($types[$type]['base64'])){
